...
export default function HeaderUser() {
    ...
    let userName=null
       if(token){
        const decoded = jwtDecode(token); // decode JWT
         userName = decoded.userName; // depends on how backend set it
      }
      ...
      // Optional: toast.error("Failed to load profile details")
    }
    finally {
 ...
 fetchUserData();
  }, [isLoading]); // Runs once when the component mounts

  // Handle Resize for Drawer
  useEffect(() => {
    const handleResize = () => {
  ...
  const handleImageUpload = (event) => {
    ...
    if (file) {
      // Create a fake URL to display the image immediately
       setPreviewImage(URL.createObjectURL(file))
      setSelectedFile(file) // Store file to send to backend later
    }
  }
  ...
  try {
    let finalImageUrl = profileImage // default: keep existing image

    // 🔴 STEP 1: Upload only if user selected a NEW image
    if (selectedFile) {

      // 🔴 STEP 2: Delete OLD image from Supabase (if exists)
      // Avoid deleting default or blob preview images
      if (
        profileImage &&
        ...
        const oldFileName = profileImage
          .split("/profiles/")[1]
          .split("?")[0] // remove cache param if exists

        const { error: deleteError } = await supabase.storage
          .from("im") // your bucket name
      ...
      // 🔴 STEP 3: Upload NEW image to Supabase
      ...
      // 🔴 STEP 4: Get PUBLIC URL + cache buster
      const { data } = supabase.storage
        .from("im")
        .getPublicUrl(filePath)

      // Cache-busting ensures browser loads the new image
      finalImageUrl = `${data.publicUrl}?t=${Date.now()}`
    }

    // 🔴 STEP 5: Prepare data for MongoDB
    const updateUser = {
      id: userData.idNumber,
      age: userData.age,
      gender: userData.sex,
      birthday: userData.birthday,
      img: finalImageUrl // Supabase image URL
    }

    // 🔴 STEP 6: Update user in backend (MongoDB)
    ...
    setProfileImage(finalImageUrl) // saved image
  ...
  return (
    <>
      ...
      <Modal
        ...
        className="md:max-w-2xl lg:w-[30%] lg:mr-45 md:mr-20 sm:w-[50%] md:w-[50%] mx-auto mt-20 bg-primary rounded-2xl shadow-2xl outline-none font-popins overflow-hidden relative" // Added 'relative'
        ...
        <div className="relative">
          ...
          <div className="px-6 pb-6 md:px-8">
            ...
            <div className="relative -mt-12 mb-4 flex justify-center items-center">
              <div className="relative group"> {/* Added relative group for hover effects */}
                <div className="border-4 border-accent rounded-full bg-white p-1 shadow-md">
                  <img
                    src={previewImage ||profileImage} // Updated to use state
                    className="w-[80px] h-[80px] rounded-full object-cover"
                    alt="Profile"
                  />
                </div>
                
                {/* --- NEW: Edit/Camera Icon --- */}
                <button 
        ...
        <div className="max-w-7xl mx-auto h-[80px] flex items-center justify-between px-4 relative">
          ...
          <div className="flex md:hidden order-1">
            <button
              ...
              className="text-2xl text-black p-2 -ml-2"
            >
              <FiMenu />
            </button>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-4 md:order-3 order-3">
            ...
            {/* Profile Avatar in Header - Updated to show new image */}
            <div
              onClick={() => setIsModalOpen(true)}
              className="w-[40px] h-[40px] md:w-[50px] md:h-[50px] rounded-full cursor-pointer ring-2 ring-transparent hover:ring-blue-500 transition"
            >
              <img
                src={profileImage} // Shows the updated image here too
      ...
      <aside
        className={`fixed top-0 left-0 h-full w-[280px] bg-white z-50 transform transition-transform duration-300 shadow-2xl
        ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="h-[80px] flex items-center justify-between px-4 border-b">
          <h2 className="font-bold text-lg text-accent">Menu</h2>
          <button onClick={() => setIsOpen(false)} className="text-2xl text-gray-500 hover:text-black">
            ✕
          </button>
        </div>
        ...
        <div className="flex flex-col gap-2 p-4">
          ...
          <div className="flex gap-2 mt-6">
            <button
              ...
              className="w-full py-2.5 rounded-full border border-accent text-accent font-medium hover:bg-amber-50"
            >
               Profile
            </button>
          </div>
